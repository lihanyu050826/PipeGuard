...
平台层将设备、管线、遥测、告警和工单抽象为统一资源，并通过 HTTP API 服务应用。原型采用内存数据便于无依赖运行；生产环境可替换为：
...
Web 大屏提供全网总览、管线监测、风险解释、设备管理、告警确认、工单处置和运营分析。界面设计优先呈现“哪里异常、异常多严重、为什么判断异常、谁来处理、处理到哪一步”。
...
原型已体现 API 输入校验、路径越界保护、线程安全数据访问及固定容量历史窗口。生产化还应增加：
