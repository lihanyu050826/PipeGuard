...
现场设备包括压力变送器、超声波流量计、可燃气体探测器和振动传感器。原型每条管线配置四类设备，生产环境可通过 Modbus RTU/TCP、OPC UA 或 HART 接入 PLC/RTU。
...
平台层将设备、管线、遥测、告警和工单抽象为统一资源，并通过 HTTP API 服务应用。原型使用 SQLite 持久化历史遥测、告警、工单和审计日志，在保持零第三方依赖的同时实现服务重启恢复；生产环境可进一步替换为：
...
Web 大屏提供全网总览、管线监测、风险解释、设备管理、告警确认、工单处置、运营分析和数据中心。界面设计优先呈现“哪里异常、异常多严重、为什么判断异常、谁来处理、处理到哪一步、数据是否已经留痕”。
